.data
    ...
    menor: .asciiz "El número menor es: "  # Define una cadena de texto para mostrar el resultado
...
.text
    main:
        ...
        blt $t0, $t1, check_t0_t2  # Compara $t0 con $t1; si $t0 < $t1, salta a la etiqueta check_t0_t2
        move $t1, $t2  # Si no, mueve el valor de $t2 a $t1

    check_t0_t2:
        blt $t0, $t2, print_menor  # Compara $t0 con $t2; si $t0 < $t2, salta a la etiqueta print_result
        move $t0, $t2  # Si no, actualiza $t0 con el valor de $t2

    print_menor:
        li $v0, 4  # Carga el código de la llamada al sistema para imprimir una cadena de texto
        la $a0, menor  # Carga la dirección de la cadena "El número menor es:"
        syscall  # Realiza la llamada al sistema para imprimir la cadena

        move $a0, $t0  # Carga el valor del número menor en el registro $a0
        li $v0, 1  # Carga el código de la llamada al sistema para imprimir un entero
        syscall  # Realiza la llamada al sistema para imprimir el número menor
